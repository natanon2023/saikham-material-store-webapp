...
<div class="main-content">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;" class=" boxmaterial">
        ...
        <a href="{{ route('technician.projects.manageproblemsindex') }}" class="btn btn-primary">ย้อนกลับ</a>
    </div>
        ...
        <tbody>
            @forelse($issues as $issue)
                <tr  style="{{ $issue->trashed() ? 'text-decoration: line-through; color: gray;' : '' }}" align="center">
                    ...
                    <td>
                        @if($issue->trashed())
                            <form action="{{ route('technician.projects.issues.restore', $issue->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('ยืนยันการกู้คืน?');">
                                @csrf
                        ...
                        @else
                            @if ($issue->status == 'pending')
                            <a href="{{ route('technician.projects.issues.edit',$issue->id) }}" class="btn-icon btn-edit" title="แก้ไข">
                                <i class="fas fa-edit"></i>
                            </a>
                            @endif
                            
                            <form action="{{ route('technician.projects.issues.destroy', $issue->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('ยืนยันการลบ?');">
                                @csrf
                                @method('DELETE')
