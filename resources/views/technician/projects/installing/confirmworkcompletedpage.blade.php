...
<div class="main-content">
    ...
    @if($project->customerneed->isNotEmpty() && $project->customerneed->every(fn($need) => !empty($need->imageafter)))
        <div style="margin-bottom: 20px; display: flex; justify-content: end; align-items: center;">
            <form action="{{ route('technician.projects.updatestatuscompleted', $project->id) }}" method="POST" style="margin: 0;">
                @csrf
        ...
        <table border="1" cellpadding="12" cellspacing="0" style="width: 100%; border-collapse: collapse; text-align: left; background: #fff;" >
            ...
            <tbody>
                ...
                <tr align = "center">
                    ...
                    <td>
                        @if(!$need->imageafter)
                            <form action="{{ route('technician.projects.uploadafterimage', $need->id) }}" method="POST" enctype="multipart/form-data" style="margin: 0;">
                                @csrf
                        ...
                        @else
                            <form action="{{ route('technician.projects.deleteafterimage', $need->id) }}" method="POST" style="margin: 0;" onsubmit="return confirm('ยืนยันการลบภาพหลังติดตั้งนี้?');">
                                @csrf
                                @method('DELETE')
