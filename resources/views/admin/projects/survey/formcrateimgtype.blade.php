...
<div class="main-content">
    ...
    <div class="box">
        <form action="{{ route('admin.projects.crateimgtype') }}" method="post">
            @csrf
    ...
    <table width="100%" border="1" style="border-collapse: collapse; margin-top: 20px;">
        ...
        <tr align="center" style ="{{ $type->trashed() ? 'opacity: 0.5; background-color: #f9f9f9;' : '' }}">
            ...
            <td width="40%">
                @if(!$type->trashed())
                    <form action="{{ route('admin.projects.updateimgtype', $type->id) }}" method="POST">
                        @csrf
                        @method('PUT')
            ...
            <td width="20%">
                @if($type->trashed())
                    <form action="{{ route('admin.projects.restoreimgtype', $type->id) }}" method="POST" style="display:inline;">
                        @csrf
                        <button type="submit" class="btn btn-secondary">กู้คืนข้อมูล</button>
                    </form>
                @else
                    <form action="{{ route('admin.projects.deleteimgtype', $type->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('ยืนยันการลบ?');">
                        @csrf
                        @method('DELETE')
