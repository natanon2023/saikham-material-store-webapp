    ...
    <div class="page">
        ...
        <div class="doc-header">
            ...
            <div class="col-right">
                ...
                <p>เลขที่ {{ $project->receipt_number ?? '-' }}</p>
                <p>อ้างอิงใบเสนอราคา {{ $project->quotation_number ?? '-' }}</p>
                <p>วันที่รับเงิน {{ \Carbon\Carbon::now()->locale('th')->addYears(543)->isoFormat('D MMMM YYYY') }}</p>
            </div>
        </div>
        <hr>
        ...
        <div class="section-title">รายการชำระเงิน (งาน {{ $project->projectname->name ?? '-' }})</div>
        <table>
            <thead>
                <tr>
                    <th width="7%">ลำดับ</th>
        ...
        <table class="sum-table-full">
            <tr>
                <td class="sum-label-full">รวมเป็นเงิน</td>
            ...
            <tr class="sum-total-full">
                <td class="sum-label-full">ยอดรับชำระสุทธิ</td>
