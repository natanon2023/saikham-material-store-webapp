...
@section('content')
    <div class="main-content">
        ...
        <div class="box-create-material" style="display: flex; justify-content: space-between; align-items: center;">
            เพิ่มประเภทอลูมิเนียมใหม่
            <div>
                <a href="{{ route('materialstype.trash') }}" class="btn btn-secondary">
                    <i class="fas fa-trash" style="margin-right: 5px;"></i>ถังขยะ
                </a> 
                <a href="{{ route('admin.materials.formaluminium') }}"
                    class="btn btn-secondary">ย้อนกลับ
                </a>
            </div>
        </div>
        <div class="box">
            <form action="{{ route('admin.materalstype.createaluminiumType') }}" method="post">
                @csrf
                <div class="box-control">
                    <div class="form-group">
                        <label class="form-label" for="">ประเภทอลูมิเนียม</label>
                        <input id="name" name="name" class="form-input" type="text"
                            placeholder="กรอกประเภทอลูมิเนียม">
                    </div>
        ...
        <div style="margin-bottom: 10px; display: flex; justify-content: space-between; align-items: center;">
            <div style="background-color: white; padding: 20px; width: auto; border: 1px solid #79c4f6;">
                ประเภทอลูมิเนียมทั้งหมด {{ $aluminiumTypes->count() }} ประเภท
            </div>
            <div style="background-color: white; padding: 20px; width: min-content; border: 1px solid #79c4f6;">
                <form action="{{ route('admin.materalstype.createaluminiumType') }}" method="get">
                    <div class="box-control" style="display: flex; align-items: center; gap: 10px;">
                        ...
                        <a class="btn btn-secondary" href="{{ route('admin.materalstype.createaluminiumType') }}"
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th style="width: auto;">ประเภทอลูมิเนียม</th>
                    <th style="width: 120px; text-align: center;">การจัดการ</th>
                </tr>
            </thead>
            <tbody>
                @forelse ($aluminiumTypes as $aluminiumType)
                    <tr>
                        <td style="text-align: center;">{{ $loop->iteration }}</td>
                        <td>{{ $aluminiumType->name }}</td>
                        <td style="text-align: center;">
                            <a href="{{ route('admin.materalstype.editAluminiumType', $aluminiumType->id) }}"
                                class="btn-icon btn-edit" title="แก้ไข">
                                <i class="fas fa-edit"></i>
                            </a>
                            <form action="{{ route('admin.materalstype.deleteAluminiumType', $aluminiumType->id) }}"
