...
@section('content')
    <div class="main-content">
        ...
        <div class="box-create-material" style="display: flex; justify-content: space-between; align-items: center;">
            เพิ่มชื่อประเภทวัสดุสิ้นเปลือง
            <div>
                <a href="{{ route('materialstype.trash') }}" class="btn btn-secondary">
                    <i class="fas fa-trash" style="margin-right: 5px;"></i>ถังขยะ
                </a> 
                <a href="{{ route('admin.materials.formconsumable') }}"
                    class="btn btn-secondary">ย้อนกลับ
                </a>
            </div>
        </div>
        <div class="box">
            <form action="{{ route('admin.materalstype.createconsumableType') }}" method="post">
                @csrf
                <div class="box-control">
                    <div class="form-group">
                        <label class="form-label" for="">ชื่อประเภทวัสดุสิ้นเปลือง</label>
                        <input id="name"  name="name" class="form-input" type="text" placeholder="กรอกชื่อประเภทวัสดุสิ้นเปลือง">
                    </div>
        ...
        <div style="margin-bottom: 10px; display: flex; justify-content: space-between; align-items: center;">
            <div style="background-color: white; padding: 20px; width: auto; border: 1px solid #79c4f6;">
                ประเภทวัสดุสิ้นเปลืองทั้งหมด {{ $consumableTypes->count() }} ประเภท
            </div>
            <div style="background-color: white; padding: 20px; width: min-content; border: 1px solid #79c4f6;">
                <form action="{{ route('admin.materalstype.createFormconsumableType') }}" method="get">
                    <div class="box-control" style="display: flex; align-items: center; gap: 10px;">
                        ...
                        <a class="btn btn-secondary" href="{{route('admin.materalstype.createFormconsumableType')}}"
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th style="width: auto;">ชื่อประเภทวัสดุสิ้นเปลือง</th>
                    <th style="width: 120px; text-align: center;">การจัดการ</th>
                </tr>
            </thead>
            <tbody>
                @forelse ($consumableTypes as $consumableType)
                    <tr>
                        <td style="text-align: center;">{{ $loop->iteration }}</td>
                        <td>{{ $consumableType->name }}</td>
                        <td style="text-align: center;">
                            <a href="{{ route('admin.materalstype.editconsumableType', $consumableType->id)}}"
                                class="btn-icon btn-edit" title="แก้ไข">
                                <i class="fas fa-edit"></i>
                            </a>
                            <form action="{{ route('admin.materalstype.deleteconsumableType', $consumableType->id) }}"
