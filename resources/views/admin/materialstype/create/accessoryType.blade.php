...
@section('content')
    <div class="main-content">
        ...
        <div class="box-create-material" style="display: flex; justify-content: space-between; align-items: center;">
            เพิ่มชื่อประเภทอุปกรณ์เสริม
            <div>
                <a href="{{ route('materialstype.trash') }}" class="btn btn-secondary">
                    <i class="fas fa-trash" style="margin-right: 5px;"></i>ถังขยะ
                </a> 
                <a href="{{ route('admin.materials.formaccessory') }}"
                    class="btn btn-secondary">ย้อนกลับ
                </a>
            </div>
        </div>
        <div class="box">
            <form action="{{ route('admin.materalstype.createaccessoryType') }}" method="post">
                @csrf
                <div class="box-control">
                    <div class="form-group">
                        <label class="form-label" >ชื่อประเภทอุปกรณ์เสริม</label>
                        <input id="name"  name="name" class="form-input" type="text" placeholder="กรอกชื่อประเภทอุปกรณ์เสริม">
                    </div>
        ...
        <div style="margin-bottom: 10px; display: flex; justify-content: space-between; align-items: center;">
            <div style="background-color: white; padding: 20px; width: auto; border: 1px solid #79c4f6;">
                ประเภทอุปกรณ์เสริมทั้งหมด {{ $accessoryTypes->count() }} ประเภท
            </div>
            <div style="background-color: white; padding: 20px; width: min-content; border: 1px solid #79c4f6;">
                <form action="{{ route('admin.materalstype.createFormaccessoryType') }}" method="get">
                    <div class="box-control" style="display: flex; align-items: center; gap: 10px;">
                        ...
                        <a class="btn btn-secondary" href="{{route('admin.materalstype.createFormaccessoryType')}}"
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th style="width: auto;">ชื่อประเภทอุปกรณ์เสริม</th>
                    <th style="width: 120px; text-align: center;">การจัดการ</th>
                </tr>
            </thead>
            <tbody>
                @forelse ($accessoryTypes as $accessoryType)
                    <tr>
                        <td style="text-align: center;">{{ $loop->iteration }}</td>
                        <td>{{ $accessoryType->name }}</td>
                        <td style="text-align: center;">
                            <a href="{{ route('admin.materalstype.editAccessoryType', $accessoryType->id)}}"
                                class="btn-icon btn-edit" title="แก้ไข">
                                <i class="fas fa-edit"></i>
                            </a>
                            <form action="{{ route('admin.materalstype.deleteAccessoryType', $accessoryType->id) }}"
